...
public sealed class CoreInfoManager
{
    ...
    public List<ECoreType> GetCheckUpdateCoreTypes()
    {
        ...
        {
            if (IsCheckUpdateSupported(ECoreType.KNcloud))
            {
                lst.Add(ECoreType.KNcloud);
            }
    ...
    public bool IsCheckUpdateSupported(ECoreType type)
    {
        return type switch
        {
            ECoreType.KNcloud => !Utils.IsPackagedInstall(),
            ECoreType.Xray => true,
    ...
    {
        var urlN = GetCoreUrl(ECoreType.KNcloud);
        ...
        [
            new CoreInfo
                {
                    CoreType = ECoreType.KNcloud,
                    Url = GetCoreUrl(ECoreType.KNcloud),
                    ReleaseApiUrl = urlN.Replace(Global.GithubUrl, Global.GithubApiUrl),
                    DownloadUrlWin64 = urlN + "/download/{0}/KNcloud-windows-64.zip",
                    DownloadUrlWinArm64 = urlN + "/download/{0}/KNcloud-windows-arm64.zip",
                    DownloadUrlLinux64 = urlN + "/download/{0}/KNcloud-linux-64.zip",
                    DownloadUrlLinuxArm64 = urlN + "/download/{0}/KNcloud-linux-arm64.zip",
                    DownloadUrlLinuxRiscV64 = urlN + "/download/{0}/KNcloud-linux-riscv64.zip",
                    DownloadUrlOSX64 = urlN + "/download/{0}/KNcloud-macos-64.zip",
                    DownloadUrlOSXArm64 = urlN + "/download/{0}/KNcloud-macos-arm64.zip",
                },
