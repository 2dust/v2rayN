...
public class CheckUpdateViewModel : MyReactiveObject
{
    ...
    private readonly ECoreType _KNcloud = ECoreType.KNcloud;
    ...
    public CheckUpdateViewModel(Func<EViewAction, object?, Task<bool>>? updateView)
    {
        ...
        CheckUpdateCmd.ThrownExceptions.Subscribe(ex =>
        {
            ...
            _ = UpdateView(_KNcloud, ex.Message);
        });
    ...
    {
        if (coreType == _KNcloud && Utils.IsPackagedInstall())
        {
            return new()
            {
                IsSelected = false,
    ...
    private async Task CheckUpdateTask()
    {
        ...
        for (var k = CheckUpdateModels.Count - 1; k >= 0; k--)
        {
            ...
            else if (item.CoreType == _KNcloud)
            {
                if (Utils.IsPackagedInstall())
                {
                    await UpdateView(_KNcloud, ResUI.MsgNotSupport);
                    continue;
                }
    ...
    private async Task CheckUpdateN(bool preRelease)
    {
        async Task _updateUI(bool success, string msg)
        {
            await UpdateView(_KNcloud, msg);
            if (success)
            {
                await UpdateView(_KNcloud, ResUI.OperationSuccess);
                UpdatedPlusPlus(_KNcloud, msg);
            }
        }
        await new UpdateService(_config, _updateUI).CheckUpdateGuiN(preRelease)
            .ContinueWith(t => UpdatedPlusPlus(_KNcloud, ""));
    }
    ...
    private async Task UpdateFinished()
    {
        if (_lstUpdated.Count > 0 && _lstUpdated.Count(x => x.IsFinished == true) == _lstUpdated.Count)
        {
            ...
            if (_lstUpdated.Any(x => x.CoreType == _KNcloud && x.IsFinished == true))
            {
    ...
    private async Task UpgradeN()
    {
        try
        {
            var fileName = _lstUpdated.FirstOrDefault(x => x.CoreType == _KNcloud)?.FileName;
            if (fileName.IsNullOrEmpty())
            {
                return;
            }
            ...
            {
                await UpdateView(_KNcloud, ResUI.UpgradeAppNotExistTip);
        ...
        {
            await UpdateView(_KNcloud, ex.Message);
        }
    }
